...
The v2 release adds a **7-component reward stack** on top of the existing deterministic grader. Each term is classified as either a mathematical guarantee or an honestly-labeled heuristic:

| # | Component | File | Type | Claim |
|---|-----------|------|------|-------|
| 1 | **Deterministic step grader** | `grader.py` | Deterministic | Multi-component tool/param/dep/reasoning scoring. Step reward in [−0.55, +0.50]. |
| 2 | **PBRS (Potential-Based Reward Shaping)** | `pbrs.py` | **Mathematical guarantee** | Optimal policy provably unchanged — see `tests/test_pbrs_invariance.py`. |
| 3 | **Count-based intrinsic motivation** | `intrinsic.py` | **Mathematical guarantee** | β/√(N+1), decays linearly to **zero by episode 200** — anti-reward-hacking by construction. |
| 4 | **Lightweight RND** | `intrinsic.py` | Heuristic | Random Network Distillation prediction error, same decay schedule as count bonus. |
| 5 | **Difference rewards** | `difference_rewards.py` | Heuristic | Counterfactual: actual step reward − baseline ("done") reward. Wired but `w=0.0` default. |
| 6 | **IRD posterior correction** | `ird.py` | Heuristic (bounded) | Posterior over 3 reward hypotheses, correction clipped to [−0.3, +0.3]. |
| 7 | **Learned judge** | `judge_model.py` | Heuristic (optional) | RandomForest soft quality signal, α=0.05 mixing. Enabled via `JUDGE_ENABLED=1`. |
| — | **RewardCombiner** | `reward_combiner.py` | Dispatch | Mode-switchable: `full / proxy_only / no_pbrs / no_intrinsic` for live ablation. |

See [`docs/REWARD_ENGINEERING.md`](docs/REWARD_ENGINEERING.md) for the full technical breakdown with per-component guarantees.
...
| `full` (extrinsic + PBRS + intrinsic) | **5.194** | 3.633 | 7.125 | **+0.712 (+15.9%)** |

![ablation chart](ablation_curve.png)

Every additional component lifts mean reward by a positive amount under a fixed policy — that's what "the term contributes" means when you've removed policy variance from the experiment. See `ablation_table.md` and `eval_ablations.py` for the harness.
...
`demo.html` now polls `GET /diagnostics` after each step and surfaces the live reward decomposition in several visual elements:

- **Reward-stack canvas** — 6-band stacked-area sparkline showing extrinsic / PBRS / intrinsic / judge / diff / IRD contributions per step.
- **Two-row breakdown panel** — extrinsic components (row 1) plus the full v2 stack with decay factor (row 2).
- **Reward mode toggle** — cycles through `full / proxy_only / no_pbrs / no_intrinsic` via `POST /diagnostics/mode`, changing reward bands in real time.
- **Sample complexity tile** — fetches `GET /metrics` and shows episodes-to-threshold training efficiency.
- **5 story cards** — `pbrsInvariant`, `intrinsicNovel`, `ablationProof`, `irdPosterior`, `sampleComplexity` trigger from real signals during the demo.
- **Episode overlay ablation panel** — shows total reward, proxy-only baseline, and the delta contributed by the reward stack.
...
| `/diagnostics` | GET | Live reward decomposition (all 7 terms + config) |
| `/diagnostics/mode` | POST | Live ablation toggle: `{"mode": "full\|proxy_only\|no_pbrs\|no_intrinsic"}` |
| `/metrics` | GET | Training metrics (sample complexity, thresholds) |
| `/docs` | GET | Swagger UI |
...
├── train.py                      ← Unsloth + TRL GRPO training (v3, bulletproof parser)
├── eval_ablations.py             ← 4-mode ablation harness
├── demo.html                     ← Live demo frontend (reward stack, mode toggle, story cards)
├── push_to_hf.py                 ← Deploy to HuggingFace Spaces
├── openenv.yaml                  ← OpenEnv manifest
├── Dockerfile
├── requirements.txt
├── docs/
│   └── REWARD_ENGINEERING.md     ← Full reward stack documentation
├── tests/
│   ├── test_pbrs_invariance.py   ← PBRS policy-invariance proof
│   ├── test_difference_rewards.py ← Difference reward correctness
│   └── test_intrinsic_decay.py   ← Intrinsic decay schedule verification
├── server/
...
└── src/envs/autopilot_env/
    ...
    ├── intrinsic.py              ← Count-based + RND intrinsic motivation
    ...
    ├── tools.py                  ← 9 mock enterprise tool APIs
